...
def test_gpu_enums():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "gpu_manager", 
            "leadfactory/services/gpu_manager.py"
        )
        ...
        sys.modules['leadfactory.services.gpu_alerting'] = type('MockModule', (), {
            'check_all_gpu_alerts': lambda *args: []
        })()
        
        sys.modules['leadfactory.services.gpu_security'] = type('MockModule', (), {
            'credential_manager': type('MockCredManager', (), {
                'get_credential': lambda self, p, t: 'mock-token'
            })(),
            'network_security': type('MockNetSec', (), {
                'load_ssh_keys': lambda self: ['mock-key']
            })(),
            'audit_logger': type('MockAudit', (), {
                'log_event': lambda self, *args, **kwargs: None
            })(),
            'rate_limiter': type('MockRate', (), {
                'check_rate_limit': lambda self, *args: True
            })()
        })()
...
def test_sql_migration():
    ...
    try:
        with open("db/migrations/add_personalization_queue.sql", "r") as f:
            sql_content = f.read()
