...
def add_leadfactory_v1_tasks():
    ...
    new_tasks = [
        ...
        },
        {
            ...
            "type": "testing",
            "created_at": timestamp,
            "dependencies": list(range(max_id + 1, max_id + 20)),  # Depends on all other tasks
    ...
    print(f"Successfully added {len(new_tasks)} new tasks (IDs {max_id + 1} to {max_id + len(new_tasks)})")
    print("\nNew tasks added:")
    for task in new_tasks:
        deps = f" (depends on: {task['dependencies']})" if task['dependencies'] else ""
        print(f"  [{task['id']}] {task['title']} - Priority: {task['priority']}{deps}")
