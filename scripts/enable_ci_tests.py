...
def convert_tests(category, min_priority_index):
    ...
    for source_file in filtered_tests:
        ...
        try:
            ...
            cmd = [
                'python', 'scripts/generate_ci_tests.py',
                '--source-dir', os.path.dirname(source_file),
                '--target-dir', os.path.dirname(target_file)
            ]
...
def generate_progress_report(status):
    ...
    with open(report_file, 'w') as f:
        ...
        enabled_tests = sum(sum(1 for test in cat['tests'].values() if test['enabled']) for cat in status['categories'].values())
        
        f.write("## Overall Progress\n\n")
        f.write(f"- Categories: {enabled_categories}/{total_categories} ({enabled_categories/total_categories*100:.1f}%)\n")
...
def main():
    ...
    parser.add_argument('--category', type=str, choices=TEST_CATEGORIES.keys(),
                        help='Test category to enable')
    parser.add_argument('--priority', type=str, choices=TEST_PRIORITIES,
                        default='high', help='Minimum test priority to enable')
