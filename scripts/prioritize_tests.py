    ...
    def discover_dependencies(self):
        ...
        for test_file in test_files:
            ...
            for func in test_funcs:
                ...
                fixture_names = re.findall(r"def\s+(\w+)\s*\(\s*\):\s*(?:[\'\"])(?:.*?)(?:[\'\"])\s*@pytest\.fixture", content)
    ...
    def generate_report(self, recommended_tests: List[Tuple[str, float]]):
        ...
        for i, (test_id, score) in enumerate(recommended_tests, 1):
            ...
            if dependencies:
                ...
                if dep_statuses:
                    report_lines.append(f"   Dependencies: {', '.join(dep_statuses[:3])}" + 
...
def main():
    ...
    parser.add_argument("--recommend", type=int, default=5, help="Number of tests to recommend for enabling")
    parser.add_argument("--enable-recommended", action="store_true", help="Enable the recommended tests automatically")
