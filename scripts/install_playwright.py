    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if deps_result.returncode == 0:
                ...
                logger.warning(
                    f"System dependencies installation had issues: {deps_result.stderr}"
                )
                logger.info(
                    "You may need to run with sudo: sudo python -m playwright install-deps chromium"
                )
...
def main():
    ...
    if verify_installation():
        ...
        logger.info(
            "Try running the script again or check the Playwright documentation"
        )
        return 1
