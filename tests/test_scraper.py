...
from utils.io import save_business, get_active_zip_codes, DatabaseConnection
...
def mock_yelp_api_for_test():
    ...
    with patch('bin.scrape.YelpAPI') as mock_yelp:
        ...
        } for i in range(1, 6)], None)  # Return 5 businesses
        yield mock_instance
...
def mock_google_places_api_for_test():
    ...
    with patch('bin.scrape.GooglePlacesAPI') as mock_google:
        ...
        mock_instance.search_places.return_value = ([{
            ...
            'name': f'Test Business {i+5}',  # Start from 6 to avoid ID collision with Yelp
...
def existing_businesses(monkeypatch):
    ...
    def mock_save_business(business_data):
        ...
        for existing in existing_businesses:
            if (business_data.get('source') == existing['source'] and 
                business_data.get('source_id') == existing['source_id']):
...
def setup_environment(monkeypatch):
    ...
    with patch('utils.io.save_business') as mock_save_business, \
        ...
        mock_get_active_zips.return_value = [{'zip': '10002', 'city': 'New York', 'state': 'NY', 'country': 'US', 'metro': 'New York'}]
...
def run_scraper(monkeypatch, setup_environment, temp_db):
    ...
    for business in existing_data:
        ...
        (name, address, city, state, zip_code, phone, website, vertical, source, source_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            business['name'], business['address'], business['city'], business['state'],
            business['zip_code'], business['phone'], business['website'],
            business['vertical'], business['source'], business['source_id']
        ))
    
    conn.commit()
    conn.close()
    # Import here to ensure environment variables are set first
    from bin.scrape import main, YelpAPI, GooglePlacesAPI
    ...
    yelp_businesses = [
        {
            'id': 'test1',
            'name': 'Test Business 1',
            'location': {'address1': '123 Test St', 'city': 'New York', 'state': 'NY', 'zip_code': '10002'},
            'phone': '+12125551001',
        ...
        },
        {
            'id': 'test2',
            'name': 'Test Business 2',
            'location': {'address1': '456 Test Ave', 'city': 'New York', 'state': 'NY', 'zip_code': '10002'},
            'phone': '+12125551002',
    ...
    with patch('bin.scrape.YelpAPI', return_value=mock_yelp) as mock_yelp_class, \
...
def existing_businesses(temp_db):
    ...
    with db_connection(temp_db) as conn:
        ...
        cursor.executemany(
            ...
            (source_id, source, name, phone, website, address, city, state, zip_code, vertical)
            VALUES (?, 'test', ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                ('existing1', 'Existing Business 1', '123-456-7890', 'https://existing1.com',
                 '123 Test St', 'Test City', 'TS', '10002', 'restaurants'),
                ('existing2', 'Existing Business 2', '987-654-3210', 'https://existing2.com',
                 '456 Test Ave', 'Test City', 'TS', '10002', 'restaurants'),
            ]
        )
        conn.commit()
    conn.close()
...
def run_scraper_yelp(mock_yelp_api_for_test, target_zip_code, target_vertical, temp_db):
    ...
    with patch('bin.scrape.YelpAPI') as mock_yelp_class, \
        ...
        mock_yelp_api_for_test.search_businesses.return_value = ([{
            ...
            'categories': [{'title': 'Restaurants'}],
            'coordinates': {'latitude': 40.7128, 'longitude': -74.0060}
        } for i in range(1, 6)], None)  # Generate 5 businesses
...
def run_scraper_google(mock_google_places_api_for_test, target_zip_code, target_vertical, temp_db):
    ...
    with patch('bin.scrape.GooglePlacesAPI') as mock_google_class, \
         patch('bin.scrape.get_zip_coordinates') as mock_coords, \
        ...
        # Mock Google Places API response with 5 businesses to match the test requirement
...
def run_scraper_step(run_scraper):
    ...
    return run_scraper['yelp'], run_scraper['google']  # Return only the API mocks for backward compatibility
...
def businesses_found(run_scraper):
    ...
    assert yelp_ok or google_ok, \
        f"Expected at least 5 businesses from either Yelp or Google, got {yelp_count} from Yelp and {google_count} from Google"
