...
class TestWebhookFinancialIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('leadfactory.services.payment_service.stripe') as mock_stripe:
            with patch('leadfactory.services.payment_service.create_engine') as mock_create_engine:
                with patch('leadfactory.services.payment_service.sessionmaker') as mock_sessionmaker:
                    ...
                    with patch('leadfactory.services.payment_service.financial_tracker', return_value=self.financial_tracker) as mock_financial_tracker:
    ...
    def test_end_to_end_payment_processing_and_aggregation(self, mock_financial_tracker, mock_stripe):
        """Test complete flow from webhook to financial tracking to aggregation."""
        # Set the mock financial tracker to our test instance
        mock_financial_tracker.record_stripe_payment = self.financial_tracker.record_stripe_payment
        mock_financial_tracker.get_daily_summary = self.financial_tracker.get_daily_summary
        mock_financial_tracker._update_daily_summary = self.financial_tracker._update_daily_summary
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM financial_transactions WHERE DATE(timestamp) = ?", (today,))
        ...
        self.assertEqual(aggregated_data['total_operational_costs_cents'], 425)  # $4.25 from costs
        ...
        self.assertAlmostEqual(aggregated_data['gross_margin_percent'], expected_gross_margin, places=2)
    ...
    @patch('leadfactory.services.payment_service.stripe')
    @patch('leadfactory.services.payment_service.financial_tracker')
    def test_refund_processing_and_aggregation(self, mock_financial_tracker, mock_stripe):
        """Test refund processing flow through to aggregation."""
        # Set the mock financial tracker to our test instance
        mock_financial_tracker.record_stripe_payment = self.financial_tracker.record_stripe_payment
        mock_financial_tracker.record_stripe_refund = self.financial_tracker.record_stripe_refund
        mock_financial_tracker.get_daily_summary = self.financial_tracker.get_daily_summary
        mock_financial_tracker._update_daily_summary = self.financial_tracker._update_daily_summary
    ...
    @patch('leadfactory.services.payment_service.stripe')
    @patch('leadfactory.services.payment_service.financial_tracker')
    def test_multiple_payments_daily_aggregation(self, mock_financial_tracker, mock_stripe):
        ...
        def mock_query_side_effect(*args, **kwargs):
            ...
            mock_query = Mock()
            mock_filter = Mock()
            mock_filter.first.return_value = mock_payments[call_count % len(mock_payments)]
            mock_query.filter.return_value = mock_filter
            call_count += 1
            return mock_query
        ...
        self.assertEqual(aggregated_data['total_operational_costs_cents'], 425)  # From setup
    ...
    def test_grafana_dashboard_data_integration(self):
        ...
        # Get dashboard data
        dashboard_data = self.aggregation_service.get_grafana_dashboard_data(today, today)
