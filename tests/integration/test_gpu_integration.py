    ...
    async def test_gpu_queue_integration(self, unified_node, mock_storage):
        ...
        with patch.object(unified_node, 'llm_client') as mock_llm:
            mock_llm.chat_completion.return_value = {
                "choices": [{"message": {"content": '{"mockup_concept": {"design_theme": "Modern"}, "email_content": {"subject": "Test"}}'}}],
                "provider": "openai",
                "model": "gpt-4o",
            ...
            assert queue_insert_call[0][1][1] == "ai_content_personalization"  # task_type
...
class TestGPUAlertIntegration:
    ...
    async def test_budget_alert_integration(self):
        ...
        with patch.object(gpu_manager, 'get_status') as mock_status, \
             patch.object(gpu_manager, 'cost_tracking', {"current_spend": 450.0, "daily_budget": 500.0}), \
    ...
    async def test_queue_alert_integration(self):
        ...
        with patch.object(gpu_manager, 'get_status') as mock_status, \
             patch.object(gpu_manager, 'cost_tracking', {"current_spend": 50.0, "daily_budget": 500.0}), \
             patch.object(gpu_manager, 'queue_metrics') as mock_queue:
    ...
    def test_alert_cooldown_integration(self):
        ...
        gpu_alert_manager.alert_cooldowns["budget_warning"] = datetime.utcnow() - timedelta(minutes=70)
        assert gpu_alert_manager._should_send_alert("budget_warning")
...
class TestGPUSecurityIntegration:
    ...
    def test_ssh_key_integration(self):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            mock_open.return_value.__enter__.return_value.read.return_value = "ssh-rsa AAAAB3NzaC1yc2E..."
...
class TestGPUMetricsIntegration:
    ...
    async def test_metrics_collection_integration(self):
        ...
        with patch('leadfactory.services.gpu_manager.METRICS_AVAILABLE', True), \
            ...
            scaling_calls = [call for call in mock_record.call_args_list 
    ...
    async def test_status_metrics_integration(self):
        ...
        with patch('leadfactory.services.gpu_manager.METRICS_AVAILABLE', True), \
            ...
            queue_calls = [call for call in mock_record.call_args_list 
...
class TestGPUEndToEnd:
    ...
    async def test_complete_gpu_workflow(self):
        ...
        with patch('leadfactory.storage.factory.get_storage') as mock_storage, \
            ...
            queue_states = [
                {"total": 500, "pending": 400, "processing": 50, "avg_time": 200, "eta": 1000, "growth_rate": 5},
                {"total": 1500, "pending": 1200, "processing": 100, "avg_time": 180, "eta": 1800, "growth_rate": 8},
                {"total": 2200, "pending": 2100, "processing": 80, "avg_time": 220, "eta": 2500, "growth_rate": 12}
            ]
            ...
            for i, queue_state in enumerate(queue_states):
                ...
                with patch.object(unified_node, 'llm_client') as mock_llm:
                    mock_llm.chat_completion.return_value = {
                        "choices": [{"message": {"content": '{"mockup_concept": {}, "email_content": {}}'}}],
                        "provider": "test", "model": "test", "usage": {}
                    }
                    
                    business_data = {
                        "id": i + 1, "name": f"Business {i+1}", "website": "test.com",
                        "description": "Test", "contact_email": "test@test.com", "industry": "technology"
                    }
