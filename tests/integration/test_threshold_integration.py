...
class TestThresholdBounceIntegration:
    ...
    def setup_method(self):
        ...
        self.bounce_monitor = BounceRateMonitor(
            db_path=self.temp_db.name, default_threshold=0.05, sampling_period_hours=24
        )

        # Create threshold configuration
        self.threshold_config = ThresholdConfig()
        self.threshold_config.add_rule(
            ThresholdRule(
        ...
        self.threshold_detector = ThresholdDetector(
            config=self.threshold_config, bounce_monitor=self.bounce_monitor
        )
    ...
    def test_end_to_end_threshold_detection(self):
        ...
        critical_breach = next(
            b for b in breaches if b.rule_name == "critical_threshold"
        )
