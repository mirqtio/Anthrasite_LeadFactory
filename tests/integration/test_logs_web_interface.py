...
from leadfactory.api.cache import LogsAPICache, get_cache, clear_cache
...
class TestLogsWebInterfaceIntegration:
    ...
    def mock_storage(self, test_db):
        ...
        def get_logs_with_filters(**kwargs):
            ...
            if sort_by == 'timestamp':
                sort_by = 'created_at'
            data_query = f'{query} ORDER BY {sort_by} {sort_order.upper()} LIMIT ? OFFSET ?'
        ...
        def get_log_statistics():
            ...
            cursor.execute('SELECT MIN(created_at), MAX(created_at) FROM raw_html_storage')
            ...
            return {
                ...
                'date_range': {
                    'earliest': min(d[0] for d in [llm_dates, html_dates] if d[0])
                        if any(d[0] for d in [llm_dates, html_dates]) else None,
                    'latest': max(d[1] for d in [llm_dates, html_dates] if d[1])
                        if any(d[1] for d in [llm_dates, html_dates]) else None
                },
                'storage_usage': {}
            }
    ...
    def test_complex_filtering_integration(self, client):
        ...
        response = client.get(f'/api/logs?business_id=2&log_type=raw_html&start_date={start_date}&end_date={end_date}&limit=10')
    ...
    def test_end_to_end_workflow_integration(self, client):
        ...
        search_response = client.post('/api/logs/search',
                                    data=json.dumps(search_data),
                                    content_type='application/json')
        ...
        export_response = client.post('/api/logs/export',
                                    data=json.dumps(export_data),
                                    content_type='application/json')
