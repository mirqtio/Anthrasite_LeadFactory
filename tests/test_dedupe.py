            ...
            with patch('bin.dedupe.get_business_by_id') as mock_get_business:
                ...
                with patch('bin.dedupe.LevenshteinMatcher') as mock_matcher_class:
                    ...
                    with patch('bin.dedupe.merge_businesses') as mock_merge:
                        ...
                        assert pair['status'] == 'merged', f"Expected status 'merged', got {pair['status']}"
                        assert pair['verified_by_llm'] == 1, "Expected verified_by_llm to be 1"
    finally:
        conn.close()
...
def test_fuzzy_matching(temp_db, mock_llm_verifier):
    ...
    with sqlite3.connect(temp_db) as conn:
        ...
        assert businesses[4]['merged_into'] == 3, "Expected business 4 to be merged into business 3"
            ...
            with patch('bin.dedupe.get_business_by_id') as mock_get_business:
                ...
                with patch('bin.dedupe.LevenshteinMatcher') as mock_matcher_class:
                    ...
                    with patch('bin.dedupe.flag_for_review') as mock_flag_for_review:
                        ...
                        assert mock_flag_for_review.called, "Expected flag_for_review to be called"
            ...
            with patch('bin.dedupe.get_business_by_id') as mock_get_business:
                ...
                with patch('bin.dedupe.LevenshteinMatcher') as mock_matcher_class:
                    ...
                    with patch('bin.dedupe.logger') as mock_logger:
                        ...
                        assert "Error processing duplicate pair" in call_args[0], f"Expected error message about processing duplicate pair, got {call_args[0]}"
    ...
    try:
        ...
        with patch('bin.dedupe.DatabaseConnection') as mock_db_conn:
            ...
            with patch('bin.dedupe.get_potential_duplicates') as mock_get_pairs:
                ...
                with patch('bin.dedupe.process_duplicate_pair', side_effect=mock_process_pair):
...
def run_deduplication(mock_llm_verifier, exact_duplicate_businesses, temp_db):
    ...
    mock_llm_verifier.verify_duplicates.return_value = (True, 0.95, "Definitely the same business")
    ...
    try:
        ...
        # Mock the merge_businesses function
        with patch('bin.dedupe.merge_businesses') as mock_merge:
            mock_merge.return_value = exact_duplicate_businesses[0]  # Return the ID of the merged business
            ...
            with patch('bin.dedupe.get_potential_duplicates') as mock_get_pairs:
                ...
                with patch('bin.dedupe.get_business_by_id') as mock_get_business:
                    ...
                    with patch('bin.dedupe.LevenshteinMatcher') as mock_matcher_class:
                        ...
                        # Verify the merge was performed correctly
                        cursor.execute("SELECT * FROM businesses WHERE id IN (?, ?)", 
                                      (exact_duplicate_businesses[0], exact_duplicate_businesses[1]))
                        businesses = {row['id']: dict(row) for row in cursor.fetchall()}
                        
                        # Second business should be merged into the first
                        assert businesses[exact_duplicate_businesses[1]]['merged_into'] == exact_duplicate_businesses[0], \
...
def run_deduplication_fuzzy(mock_llm_verifier, similar_businesses, temp_db):
    ...
    # Mock the LLM verifier to confirm duplicates
    mock_llm_verifier.verify_duplicates.return_value = (True, 0.85, "Likely the same business")
