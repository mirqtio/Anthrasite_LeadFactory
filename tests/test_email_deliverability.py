...
def mock_sendgrid_api():
    ...
    with patch('requests.get') as mock_get, patch('requests.post') as mock_post:
        ...
        mock_get.return_value = MagicMock(
            status_code=200,
            json=lambda: []  # Empty stats by default
        )
...
def attempt_send_email(email_sender, mock_sendgrid_api):
    ...
    if should_block:
        ...
        email_sender.send_result = (False, None, f"Metrics exceed thresholds: bounce_rate={test_bounce_rate:.2%}, spam_rate={test_spam_rate:.4%}")
    else:
