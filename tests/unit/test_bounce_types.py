...
class TestSoftBounces:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "450 4.2.2 The email account that you tried to reach is over quota",
                "4.2.2",
            ),
...
class TestDroppedEvents:
    ...
    def test_dropped_event_processing(self):
        ...
        # Act
        with patch.object(
            self.webhook_handler, "_process_bounce_event"
        ) as mock_process:
    ...
    def test_unknown_bounce_type_handling(self):
        ...
        with patch.object(self.webhook_handler, "_store_bounce_event"):
            try:
                ...
                pytest.fail(
                    f"Processing unknown bounce types should not raise exceptions: {e}"
                )
