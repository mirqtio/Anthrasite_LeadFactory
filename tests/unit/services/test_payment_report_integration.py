...
from leadfactory.services.payment_service import StripePaymentService, Payment, PaymentStatus, PaymentConfig
...
class TestPaymentReportIntegration:
    ...
    def mock_report_delivery(self):
        ...
        with patch('leadfactory.services.payment_service.ReportDeliveryService') as mock:
    ...
    def payment_service(self, payment_config, mock_db_session, mock_report_delivery):
        ...
        with patch('leadfactory.services.payment_service.create_engine'), \
            ...
            mock_session_local.return_value.__enter__ = Mock(return_value=mock_db_session)
    ...
    def test_successful_payment_triggers_report_delivery(
        self, payment_service, mock_db_session, mock_report_delivery, mock_stripe, sample_payment
    ):
        ...
        with patch('leadfactory.services.payment_service.financial_tracker') as mock_tracker:
        ...
        assert updated_metadata['storage_path'] == "reports/test@example.com/audit_financial_payment-123_1234567890.pdf"
    ...
    def test_payment_success_with_report_delivery_failure(
        ...
        mock_report_delivery.upload_and_deliver_report.side_effect = Exception("Report delivery failed")
    ...
    def test_pdf_generation_includes_payment_details(self, payment_service, sample_payment):
    ...
    def test_webhook_signature_verification_failure(
        self, payment_service, mock_stripe
    ):
    ...
    def test_multiple_audit_types_generate_different_reports(
        ...
        for audit_type in audit_types:
            ...
            mock_stripe.Charge.list.return_value.data = [{"id": "ch_test", "amount": 9900, "balance_transaction": "txn_test"}]
            mock_stripe.BalanceTransaction.retrieve.return_value = {"fee": 317, "net": 9583}
            mock_report_delivery.upload_and_deliver_report.return_value = {"status": "delivered"}
