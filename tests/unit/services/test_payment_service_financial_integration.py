...
from leadfactory.services.payment_service import PaymentStatus, StripePaymentService
...
class TestPaymentServiceFinancialIntegration(unittest.TestCase):
    ...
    @patch("leadfactory.services.payment_service.financial_tracker")
    @patch("leadfactory.services.payment_service.stripe")
    def test_payment_succeeded_handles_missing_balance_transaction(
        self, mock_stripe, mock_financial_tracker
    ):
    ...
    @patch("leadfactory.services.payment_service.financial_tracker")
    @patch("leadfactory.services.payment_service.stripe")
    def test_payment_succeeded_handles_stripe_api_error(
        self, mock_stripe, mock_financial_tracker
    ):
    ...
    @patch("leadfactory.services.payment_service.financial_tracker")
    @patch("leadfactory.services.payment_service.stripe")
    def test_refund_created_records_financial_data(
        self, mock_stripe, mock_financial_tracker
    ):
    ...
    @patch("leadfactory.services.payment_service.financial_tracker")
    @patch("leadfactory.services.payment_service.stripe")
    def test_refund_created_handles_balance_transaction_error(
        self, mock_stripe, mock_financial_tracker
    ):
    ...
    def test_webhook_handler_routes_refund_events(self, mock_construct_event):
        ...
        with patch.object(
            self.payment_service, "_handle_refund_created"
        ) as mock_refund_handler:
    ...
    def test_chargeback_created_updates_payment_status(self):
        ...
        dispute = {"charge": "ch_test_123", "payment_intent": "pi_test_123"}
