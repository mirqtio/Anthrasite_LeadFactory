...
class TestReportDeliveryService:
    ...
    def service(self, mock_storage, mock_link_generator, mock_email_service):
        ...
        return ReportDeliveryService(storage_bucket="test-reports", link_expiry_hours=24)
    ...
    def test_service_initialization(self, mock_storage, mock_link_generator, mock_email_service):
        """Test service initialization with custom parameters."""
        with patch('leadfactory.services.report_delivery.SupabaseStorage') as mock_storage_class, \
             patch('leadfactory.services.report_delivery.SecureLinkGenerator') as mock_link_class, \
             patch('leadfactory.services.report_delivery.EmailDeliveryService') as mock_email_class:

            service = ReportDeliveryService(storage_bucket="custom-bucket", link_expiry_hours=48)
    ...
    def test_upload_and_deliver_report_success(
        self, service, mock_storage, mock_link_generator, mock_email_service, sample_pdf_path
    ):
    ...
    def test_upload_and_deliver_report_with_custom_expiry(
        ...
        mock_link_generator.generate_secure_link.assert_called_once()
        expiry_days_arg = mock_link_generator.generate_secure_link.call_args[1]['expiry_days']
    ...
    def test_validate_and_get_report_access_invalid_token(
        ...
        mock_link_generator.validate_secure_link.side_effect = Exception("Invalid token")
    ...
    def test_get_delivery_status_found(self, service, mock_storage):
        ...
        assert result['latest_file']['name'] == "reports/user123/audit_report_123_456789.pdf"
    ...
    def test_cleanup_expired_reports_success(self, service, mock_storage):
        ...
        mock_storage.list_files.return_value = [
            {
                "name": "reports/user1/old_report.pdf",
                "created_at": old_date
            },
            {
                "name": "reports/user2/recent_report.pdf",
                "created_at": recent_date
            },
            {
                "name": "reports/user3/very_old_report.pdf",
                "created_at": old_date
            }
        ]
    ...
    def test_cleanup_expired_reports_with_errors(self, service, mock_storage):
        ...
        mock_storage.list_files.return_value = [
            {
                "name": "reports/user1/old_report1.pdf",
                "created_at": old_date
            },
            {
                "name": "reports/user2/old_report2.pdf",
                "created_at": old_date
            }
        ]
