...
class TestGPUAutoSpinManager:
    ...
    def gpu_manager(self):
        ...
        with (
            patch("leadfactory.services.gpu_manager.credential_manager"),
    ...
    async def test_start_hetzner_instance(self, gpu_manager):
        ...
        mock_response = {
            "server": {"id": 12345, "name": "test-server", "status": "initializing"}
        }
        ...
        with (
            ...
            instance_id = await gpu_manager._start_hetzner_instance(
...
class TestGPUManagerIntegration:
    ...
    async def test_full_scaling_cycle(self):
        ...
        with (
            ...
            # Scale up
            instance_id = await manager.start_gpu_instance(
                GPUInstanceType.HETZNER_GTX1080
            )
            assert instance_id == "12345"
