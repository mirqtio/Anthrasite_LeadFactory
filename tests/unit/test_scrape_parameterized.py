...
@pytest.mark.parametrize(
    "category_case", test_categories, ids=[case["id"] for case in test_categories]
)
def test_scraping_by_category(category_case):
    ...
    with (
        patch("bin.scrape.scrape_businesses_from_api") as mock_scrape,
        ...
        results = scrape.scrape_businesses_by_category(
            category_case["category"], location, limit
        )
...
test_locations = [
    ...
    {"id": "zip_code", "location": "90210", "radius": 5, "expected_min_businesses": 10},
    {
        "id": "county",
...
@pytest.mark.parametrize(
    "location_case", test_locations, ids=[case["id"] for case in test_locations]
)
def test_scraping_by_location(location_case):
            ...
            businesses.append(
                {
                    ...
                    "distance": i
                    * (
                        radius / location_case["expected_min_businesses"]
                    ),  # Simulate distance within radius
                }
            )
        return businesses
    ...
    with patch("bin.scrape.scrape_businesses_from_api_with_radius") as mock_scrape:
        ...
        results = scrape.scrape_businesses_by_location(
            category, location_case["location"], location_case["radius"], limit
        )
...
@pytest.mark.parametrize(
    "filter_case", test_filters, ids=[case["id"] for case in test_filters]
)
def test_business_filtering(filter_case):
    ...
    if filter_case["filter_type"] == "has_website":
        ...
        filtered = [
            b for b in filter_case["businesses"] if location_filter(b, location)
        ]
        ...
        for filter_item in filter_case["filters"]:
            if filter_item["type"] == "has_website":
                ...
                filtered = [
                    b for b in filtered if keyword_filter(b, filter_item["value"])
                ]
            elif filter_item["type"] == "location":
                filtered = [
                    b for b in filtered if location_filter(b, filter_item["value"])
                ]
...
@pytest.mark.parametrize(
    "error_case", error_test_cases, ids=[case["id"] for case in error_test_cases]
)
    ...
    with (
        ...
        try:
            ...
            def scrape_with_retry(
                category, location, limit, max_retries=3, backoff_factor=1.5
            ):
                """Simplified version of a function that retries scraping on certain errors."""
                retries = 0
                while retries <= max_retries:
                    try:
                        return scrape.scrape_businesses_from_api(
