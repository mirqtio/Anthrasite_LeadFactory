...
class TestPostgresStorage:
    ...
    def test_email_unsubscribe_workflow(self):
        ...
        if IMPORTS_AVAILABLE:
            with patch('leadfactory.utils.e2e_db_connector.db_connection') as mock_db_connection:
                mock_conn = Mock()
                mock_cursor = Mock()

                # Set up the cursor behavior for the test sequence
                mock_cursor.fetchone.side_effect = [
                    None,  # First call: is_email_unsubscribed returns None (not unsubscribed)
                    None,  # Second call: add_unsubscribe check returns None (not already unsubscribed)
                    {"id": 1}  # Third call: is_email_unsubscribed returns record (unsubscribed)
                ]
                mock_cursor.rowcount = 1  # For add_unsubscribe success
    ...
    def test_email_logging(self):
        ...
        if IMPORTS_AVAILABLE:
            with patch('leadfactory.utils.e2e_db_connector.db_connection') as mock_db_connection:
                ...
                mock_cursor.rowcount = 1  # For successful inserts
