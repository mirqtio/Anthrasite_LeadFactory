...
from leadfactory.api.cache import CacheEntry, LogsAPICache
...
class TestCacheEntry:
    ...
    def test_cache_entry_creation(self):
        ...
        entry = CacheEntry(
            data={"test": "data"}, created_at=created_at, ttl_seconds=300
        )
    ...
    def test_cache_entry_access(self):
        ...
        entry = CacheEntry(
            data="test_data", created_at=datetime.utcnow(), ttl_seconds=300
        )
    ...
    def test_cache_entry_expiration(self):
        ...
        entry_no_expire = CacheEntry(
            data="test", created_at=datetime.utcnow(), ttl_seconds=0
        )
        assert not entry_no_expire.is_expired()

        # Expired entry
        past_time = datetime.utcnow() - timedelta(seconds=600)
        entry_expired = CacheEntry(data="test", created_at=past_time, ttl_seconds=300)
        assert entry_expired.is_expired()

        # Fresh entry
        entry_fresh = CacheEntry(
            data="test", created_at=datetime.utcnow(), ttl_seconds=300
        )
...
class TestLogsAPICache:
    ...
    def test_logs_query_caching(self):
        ...
        result = cache.get_logs_with_filters(
            business_id=123, log_type="llm", limit=50, offset=0
        )
    ...
    def test_statistics_caching(self):
        ...
        test_stats = {"total_logs": 1000, "logs_by_type": {"llm": 600, "html": 400}}
    ...
    def test_cache_key_generation(self):
        ...
        cache.set_logs_with_filters((["log1"], 1), business_id=123, log_type="llm")

        result1 = cache.get_logs_with_filters(business_id=123, log_type="llm")
        result2 = cache.get_logs_with_filters(
            log_type="llm", business_id=123
        )  # Different order
...
class TestCacheIntegration:
    ...
    def test_search_query_ttl_adjustment(self):
        ...
        cache.set_logs_with_filters((["log"], 1), search_query="test search", limit=50)
