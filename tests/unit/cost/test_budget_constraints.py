...
try:
    ...
    class BudgetConstraints:
        ...
        def can_execute_operation(self, service, operation, parameters=None):
            return True, "Budget allows execution", [self.estimate_operation_cost(service, operation, parameters)]
...
class TestBudgetConstraints:
    ...
    def test_estimate_gpu_operation_cost(self):
        ...
        estimate = self.budget_constraints.estimate_operation_cost(
            "gpu", "hourly-rate", {"hours": 1}
        )

        assert estimate.service == "gpu"
        assert estimate.operation == "hourly-rate"
        assert estimate.estimated_cost > 0
    ...
    def test_estimate_unknown_service(self):
        ...
        assert estimate.estimated_cost == 0.0  # Unknown service returns 0.0
    ...
    def test_can_execute_operation_within_budget(self):
        ...
        can_execute, reason, cost_estimates = self.budget_constraints.can_execute_operation(
        ...
        assert cost_estimates[0].service == "openai"
        assert cost_estimates[0].operation == "gpt-4o"
    ...
    def test_get_status_level(self):
        ...
        # Create mock BudgetStatus objects for different scenarios
        from leadfactory.cost.budget_constraints import BudgetStatus, BudgetLimit

        # Create a budget limit for testing
        limit = BudgetLimit(name="daily", limit_amount=100.0, period="daily")

        # Test normal status
        normal_status = BudgetStatus(
            current_spent=50.0, limit=limit, remaining=50.0,
            utilization_percent=50.0, is_warning=False, is_critical=False, is_exceeded=False
        )
        assert self.budget_constraints._get_status_level(normal_status) == "normal"

        # Test warning status
        warning_status = BudgetStatus(
            current_spent=85.0, limit=limit, remaining=15.0,
            utilization_percent=85.0, is_warning=True, is_critical=False, is_exceeded=False
        )
        assert self.budget_constraints._get_status_level(warning_status) == "warning"

        # Test critical status
        critical_status = BudgetStatus(
            current_spent=96.0, limit=limit, remaining=4.0,
            utilization_percent=96.0, is_warning=True, is_critical=True, is_exceeded=False
        )
        ...
        exceeded_status = BudgetStatus(
            current_spent=110.0, limit=limit, remaining=-10.0,
            utilization_percent=110.0, is_warning=True, is_critical=True, is_exceeded=True
        )
    ...
    def test_estimate_cost(self):
        ...
        if IMPORTS_AVAILABLE:
            with patch('leadfactory.cost.budget_constraints.budget_constraints') as mock_bc:
                ...
                # estimate_cost returns just the cost float, not the full CostEstimate object
                assert cost == 0.05
