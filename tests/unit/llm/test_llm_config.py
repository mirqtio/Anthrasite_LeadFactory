...
class TestLLMConfig:
    ...
    def test_should_fallback_on_error(self):
        ...
        assert config.should_fallback_on_error(
...
class TestConfigFromEnvironment:
    ...
    def test_openai_config_from_env(self):
        ...
        with patch.dict(os.environ, env_vars, clear=False):
            with patch('leadfactory.llm.config.OPENAI_API_KEY', 'test-openai-key'), \
    ...
    def test_anthropic_config_from_env(self):
        ...
        with patch.dict(os.environ, env_vars, clear=False):
            with patch('leadfactory.llm.config.ANTHROPIC_API_KEY', 'test-anthropic-key'), \
                 patch('leadfactory.llm.config.ANTHROPIC_MODEL', 'claude-3-opus-20240229'):
    ...
    def test_ollama_config_from_env(self):
        ...
        with patch.dict(os.environ, env_vars, clear=False):
            with patch('leadfactory.llm.config.OLLAMA_HOST', 'http://localhost:11434'), \
    ...
    def test_global_overrides_from_env(self):
        ...
        with patch.dict(os.environ, env_vars, clear=False):
            with patch('leadfactory.llm.config.get_int_env') as mock_get_int, \
    ...
    def test_no_providers_configured(self):
        ...
        # Clear all API keys
        with patch('leadfactory.llm.config.OPENAI_API_KEY', None), \
