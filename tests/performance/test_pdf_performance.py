...
class TestPDFPerformance:
    ...
    def test_large_report_generation_performance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert generation_time < 120.0, f"Large report generation took too long: {generation_time:.2f}s"
    ...
    def test_memory_usage_performance(self):
        ...
        assert total_memory_increase < 500.0, f"Memory usage increased too much: {total_memory_increase:.2f}MB"
    ...
    def test_concurrent_generation_performance(self):
        ...
        for result in results:
            ...
            assert result["generation_time"] < 60.0, f"Thread {result['thread_id']} too slow"
...
class TestPDFBenchmarks:
    ...
    def test_baseline_performance_benchmark(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert generation_time < 30.0, f"Baseline generation time: {generation_time:.2f}s (target: <30s)"
            assert file_size_mb < 20.0, f"Baseline file size: {file_size_mb:.2f}MB (target: <20MB)"
            assert len(pdf_bytes) > 0
