...
class TestRunNightly(unittest.TestCase):
    ...
    def test_batch_completion_metric(self, mock_run, mock_open, mock_exists):
        ...
        expected_calls = [
            call(["python", "-c", "import sys; sys.path.append('.'); from bin.metrics import metrics; metrics.batch_completed_timestamp.set(time.time()); metrics.observe_batch_duration($DURATION)"], 
                 cwd=self.test_dir, 
                 env=env, 
                 shell=True, 
                 check=True)
    ...
    def test_supabase_usage_check(self, mock_run):
        ...
        expected_call = call(["python", "scripts/monitor_supabase_usage.py", "--alert-threshold", "80"], 
                            cwd=self.test_dir, 
                            check=True)
