...
def test_run_nightly_script_exists():
    ...
    assert os.access(RUN_NIGHTLY_SCRIPT, os.X_OK), (
...
def test_run_nightly_skip_stage():
    ...
    assert "--skip-stage=*" in script_content, (
    ...
    assert 'run_stage 5 "Mockup Generation"' in script_content, (
...
def test_setup_cron_script_exists():
    ...
    assert os.access(setup_cron_script, os.X_OK), (
...
def test_script_path_consistency():
    ...
    for script_path in script_paths:
        ...
        if "_" in base_name:
            ...
            assert os.path.exists(actual_path) or os.path.exists(non_prefixed_path), (
                error_message
            )
