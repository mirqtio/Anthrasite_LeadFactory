...
try:
    ...
    class MockBudgetGate:
        ...
        def get_current_month_costs(db_conn):
            ...
            cursor.execute(
                "SELECT SUM(cost) FROM api_costs WHERE strftime('%Y-%m', timestamp) = strftime('%Y-%m', 'now')"
            )
        ...
        def get_current_day_costs(db_conn):
            ...
            cursor = db_conn.cursor()
            cursor.execute(
                "SELECT SUM(cost) FROM api_costs WHERE date(timestamp) = date('now')"
            )
            result = cursor.fetchone()
            return result[0] if result and result[0] else 0.0
        ...
        def check_budget_status(db_conn):
            ...
            cursor = db_conn.cursor()
            cursor.execute(
                "SELECT monthly_budget, warning_threshold, pause_threshold FROM budget_settings LIMIT 1"
            )
            ...
            usage_percentage = (
                monthly_costs / monthly_budget if monthly_budget > 0 else 0.0
            )
...
def check_monthly_cost(api_costs_logged):
    ...
    assert monthly_costs == 0.30, (
        f"Monthly costs should be 0.30, got {monthly_costs}"
    )  # 0.10 + 0.05 + 0.15
...
def check_cost_by_model(api_costs_logged):
    ...
    assert by_model["gpt-4"] == 0.25, (
        f"GPT-4 cost should be 0.25, got {by_model['gpt-4']}"
    )  # 0.10 + 0.15
    assert by_model["gpt-3.5-turbo"] == 0.05, (
        f"GPT-3.5-turbo cost should be 0.05, got {by_model['gpt-3.5-turbo']}"
    )
...
def check_cost_by_purpose(api_costs_logged):
    ...
    assert by_purpose["verification"] == 0.10, (
        f"Verification cost should be 0.10, got {by_purpose['verification']}"
    )
    assert by_purpose["email"] == 0.05, (
        f"Email cost should be 0.05, got {by_purpose['email']}"
    )
    assert by_purpose["mockup"] == 0.15, (
        f"Mockup cost should be 0.15, got {by_purpose['mockup']}"
    )
...
def check_budget_limits(api_costs_logged):
    ...
    assert status["status"] in ["active", "warning", "paused"], (
        f"Status should be one of active, warning, paused, got {status['status']}"
    )
    ...
    assert status["usage_percentage"] < 0.5, (
        f"Usage percentage should be less than 50%, got {status['usage_percentage'] * 100}%"
    )
