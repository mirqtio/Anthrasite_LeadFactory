...
def test_tech_stack_rule(sample_rules_file):
    ...
    business_data = {
        ...
        "features": {
            "tech_stack": '{"WordPress": {"version": "5.8"}, "PHP": {"version": "7.2.0"}}'
        }
    }
    
    score, applied_rules = engine.calculate_score(business_data)
    
    # Base (50) + WordPress (10) + Outdated PHP (15) = 75 (PHP 7.2 is considered outdated)
    assert score == 75
...
def test_multiple_rules(sample_rules_file):
    ...
    # Base (50) + WordPress (10) + Outdated PHP (15) + Good Performance (-15) + Target Location (5) = 65
    assert score == 65
...
def test_multiplier(sample_rules_file):
    ...
    # Base (50) * 1.5 (healthcare) * 1.2 (multiple locations) = 89.999... -> 89 (floating point)
