...
def test_scrape_businesses(target_zip_code, target_vertical, temp_db, monkeypatch, capsys):
    ...
    def mock_process_yelp(business, category):
        with db_connection(temp_db) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO businesses (name, phone, email, website, vertical, source, source_id)
                VALUES (?, ?, ?, ?, ?, 'yelp', ?)
            """, (
                business.get('name', ''),
                business.get('phone', ''),
    ...
    with db_connection(temp_db) as conn:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) FROM businesses WHERE source = 'yelp'"
        )
        yelp_count = cursor.fetchone()[0]

        cursor = conn.execute(
            "SELECT COUNT(*) FROM businesses WHERE source = 'google'"
        )
...
def test_main(target_vertical, temp_db, monkeypatch, capsys):
    ...
    def mock_parse_args():
        class Args:
            limit = 2
            zip = None  # Changed from zip_code to zip to match the argument name in main()
