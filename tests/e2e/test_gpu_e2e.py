...
from leadfactory.services.gpu_manager import gpu_manager, GPUInstanceType
...
class TestGPUAutoScalingE2E:
    ...
    async def test_complete_scaling_workflow(self):
        ...
        with patch('leadfactory.storage.factory.get_storage') as mock_storage, \
            ...
            queue_progression = [
                ...
                {"total": 50, "pending": 30, "processing": 15, "avg_time": 200, "eta": 400, "growth_rate": 2},
                # Growing queue - approaching threshold
                {"total": 1800, "pending": 1500, "processing": 200, "avg_time": 180, "eta": 1350, "growth_rate": 8},
                # Large queue - should trigger scaling
                {"total": 2300, "pending": 2100, "processing": 150, "avg_time": 220, "eta": 2500, "growth_rate": 15},
                # Processing queue - high utilization
                {"total": 2400, "pending": 1800, "processing": 500, "avg_time": 190, "eta": 1800, "growth_rate": 10},
                # Queue processed - should trigger scale down
                {"total": 200, "pending": 50, "processing": 20, "avg_time": 150, "eta": 375, "growth_rate": 1}
            ]
            ...
            for i, queue_state in enumerate(queue_progression):
                ...
                if queue_state["pending"] > 2000:
                    ...
                    if gpu_manager._within_budget_to_scale() and gpu_manager._can_scale_up():
                        assert len(gpu_manager.active_instances) >= 1, f"Should have scaled up at step {i+1}"
                ...
                elif queue_state["pending"] < 100 and len(gpu_manager.active_instances) > 1:
    ...
    async def test_pipeline_integration_e2e(self):
        ...
        with patch('leadfactory.storage.factory.get_storage') as mock_storage, \
            ...
            storage.execute_query.side_effect = [
                [{"id": task_id}] for task_id in range(1, 101)  # 100 tasks
            ]
            ...
            with patch.object(unified_node, 'llm_client') as mock_llm:
                mock_llm.chat_completion.return_value = {
                    "choices": [{"message": {"content": '{"mockup_concept": {"design_theme": "Modern"}, "email_content": {"subject": "Test Subject", "greeting": "Hello"}}'}}],
                    "provider": "openai",
                    "model": "gpt-4o", 
                ...
                for i in range(50):  # Process 50 businesses
                    ...
                    with patch.object(unified_node, '_fetch_business_data', return_value=business_data):
                ...
                assert len(successful_results) == 50, "All businesses should be processed successfully"
                
                # Simulate queue metrics after processing
                with patch('leadfactory.services.gpu_manager.get_queue_metrics') as mock_metrics:
                    ...
                    if gpu_manager._within_budget_to_scale() and gpu_manager._can_scale_up():
                        assert len(gpu_manager.active_instances) > 0, "Should have scaled up GPU instances"
    ...
    async def test_alert_system_e2e(self):
        ...
        with patch('leadfactory.services.gpu_manager.ALERTING_AVAILABLE', True):
            ...
            assert second_alert_time <= first_alert_time + 1, "Cooldown should prevent alert spam"
    ...
    def test_security_system_e2e(self):
        ...
        assert retrieved_token == "e2e-test-token", "Should retrieve correct credentials"
    ...
    async def test_cost_optimization_e2e(self):
        ...
        with patch.object(gpu_manager, 'hetzner_client') as mock_hetzner, \
             patch('leadfactory.services.gpu_manager.SECURITY_AVAILABLE', False):
            
            # Mock Hetzner responses for different instance types
            instance_responses = {
                GPUInstanceType.HETZNER_GTX1080: {"server": {"id": 1001, "name": "gtx1080"}},
                GPUInstanceType.HETZNER_RTX3080: {"server": {"id": 1002, "name": "rtx3080"}}, 
                GPUInstanceType.HETZNER_RTX4090: {"server": {"id": 1003, "name": "rtx4090"}}
            }
            
            mock_hetzner.create_server.side_effect = lambda *args, **kwargs: instance_responses[
            ...
            for queue_size, expected_type in test_cases:
                ...
                with patch('leadfactory.services.gpu_manager.get_queue_metrics') as mock_metrics:
                    ...
                    mock_hetzner.create_server.return_value = instance_responses[expected_type]
                    ...
                    if len(gpu_manager.active_instances) > 0:
                        ...
                        print(f"Queue size {queue_size} -> Instance type {instance.instance_type}")
    ...
    async def test_failure_recovery_e2e(self):
        ...
        with patch.object(gpu_manager, 'hetzner_client') as mock_hetzner, \
            ...
            instance_id = await gpu_manager.start_gpu_instance(GPUInstanceType.HETZNER_GTX1080)
            ...
            mock_hetzner.create_server.return_value = {"server": {"id": 2001, "name": "recovery-test"}}
