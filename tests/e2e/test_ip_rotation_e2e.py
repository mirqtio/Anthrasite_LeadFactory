...
class TestIPRotationE2E:
    ...
    def integrated_system(
        self, temp_db, bounce_config, threshold_config, rotation_config
    ):
    ...
    def test_complete_rotation_workflow_on_high_bounce_rate(self, integrated_system):
        ...
        rotation_result = rotation_service.rotate_ip(
            primary_ip, primary_subuser, RotationReason.HIGH_BOUNCE_RATE
        )
        ...
        original_pool = rotation_service.get_ip_subuser_pool(
            primary_ip, primary_subuser
        )
    ...
    def test_cooldown_period_prevents_immediate_reuse(self, integrated_system):
        ...
        available_ips = [
            pool for pool in rotation_service.ip_pool if pool.is_available()
        ]
