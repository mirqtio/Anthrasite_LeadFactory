...
class DockerComposeManager:
    ...
    def stop_services(self):
        """Stop all services."""
        subprocess.run(
            ["docker-compose", "-f", self.compose_file, "down", "-v"], check=True
        )

    def restart_service(self, service: str):
        """Restart a specific service."""
        subprocess.run(
            ["docker-compose", "-f", self.compose_file, "restart", service], check=True
        )
...
class TestMicroservicesIntegration:
    ...
    async def test_api_gateway_routing(self, http_client, docker_manager):
        ...
        endpoints = ["/api/scrape", "/api/enrich", "/api/score", "/api/email"]
    ...
    async def test_database_connectivity(self, http_client, docker_manager):
        ...
        endpoints_with_db = ["/api/scrape", "/api/score"]
        ...
        for endpoint in endpoints_with_db:
            async with http_client.get(f"{base_url}{endpoint}/health") as response:
                ...
                async with http_client.post(
                    f"{base_url}{endpoint}", json=test_payload
                ) as write_response:
                    # Should be able to process requests that involve database
                    assert write_response.status in [
                        200,
                        202,
                        400,
                    ]  # 400 for invalid test payload
    ...
    def test_service_logs_for_errors(self, docker_manager):
        ...
        for service in services:
            ...
            for error_pattern in critical_errors:
                assert error_pattern not in logs, (
                    f"Critical error found in {service} logs: {error_pattern}"
                )
    ...
    async def test_performance_under_load(self, http_client, docker_manager):
        ...
        successful_requests = len(
            [r for r in results if isinstance(r, tuple) and r[0] in [200, 202]]
        )
        throughput = successful_requests / total_time

        logger.info(
            f"Performance test: {successful_requests}/50 successful, {throughput:.2f} RPS"
        )

        # Basic performance assertions
        assert successful_requests >= 45, (
            "Should have high success rate under moderate load"
        )
