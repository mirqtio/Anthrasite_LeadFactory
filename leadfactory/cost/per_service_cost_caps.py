...
class PerServiceCostCaps:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Per-service cost caps initialized with {len(self.service_caps)} services"
        )
        ...
        default_caps = {
            "openai": ServiceCostCap(
                ...
                warning_threshold=float(
                    os.environ.get("OPENAI_WARNING_THRESHOLD", "0.8")
                ),
                critical_threshold=float(
                    os.environ.get("OPENAI_CRITICAL_THRESHOLD", "0.9")
                ),
            ),
            "semrush": ServiceCostCap(
                ...
                warning_threshold=float(
                    os.environ.get("SEMRUSH_WARNING_THRESHOLD", "0.8")
                ),
                critical_threshold=float(
                    os.environ.get("SEMRUSH_CRITICAL_THRESHOLD", "0.9")
                ),
            ),
            "screenshot": ServiceCostCap(
                ...
                warning_threshold=float(
                    os.environ.get("SCREENSHOT_WARNING_THRESHOLD", "0.8")
                ),
                critical_threshold=float(
                    os.environ.get("SCREENSHOT_CRITICAL_THRESHOLD", "0.9")
                ),
            ),
            "gpu": ServiceCostCap(
                service="gpu",
                ...
                critical_threshold=float(
                    os.environ.get("GPU_CRITICAL_THRESHOLD", "0.9")
                ),
            ),
            "sendgrid": ServiceCostCap(
                ...
                warning_threshold=float(
                    os.environ.get("SENDGRID_WARNING_THRESHOLD", "0.8")
                ),
                critical_threshold=float(
                    os.environ.get("SENDGRID_CRITICAL_THRESHOLD", "0.9")
                ),
            ),
        }
    ...
    def get_service_status(self, service: str) -> ServiceCostStatus:
        ...
        utilization_percent = (
            (daily_spent / cap.daily_limit * 100) if cap.daily_limit > 0 else 0.0
        )
    ...
    def can_execute_operation(
        self, service: str, estimated_cost: float
        ...
        if status.status == ServiceStatus.CAPPED:
            return (
                False,
                f"Service {service} has exceeded daily cap of ${status.daily_limit:.2f}",
                status,
            )
    ...
    def get_cost_cap_report(self) -> dict[str, any]:
        ...
        total_daily_limit = sum(
            cap.daily_limit for cap in self.service_caps.values() if cap.enabled
        )
        total_daily_spent = sum(status.daily_spent for status in statuses.values())

        capped_count = len(
            [s for s in statuses.values() if s.status == ServiceStatus.CAPPED]
        )
        warning_count = len(
            [
                s
                for s in statuses.values()
                if s.status in [ServiceStatus.WARNING, ServiceStatus.CRITICAL]
            ]
        )
        ...
        return {
            ...
            "summary": {
                ...
                "overall_utilization_percent": (
                    (total_daily_spent / total_daily_limit * 100)
    ...
    def estimate_remaining_capacity(self, service: str) -> dict[str, any]:
        ...
        total_operations = sum(1 for _ in service_breakdown)  # Simple count
        avg_cost_per_operation = (
            status.daily_spent / total_operations if total_operations > 0 else 0.1
        )

        estimated_operations = (
            int(status.remaining / avg_cost_per_operation)
        ...
        if hours_elapsed > 0 and status.daily_spent > 0:
            ...
            hours_until_cap = (
                status.remaining / hourly_burn_rate
        ...
        return {
            ...
            "current_burn_rate": (
                status.daily_spent / hours_elapsed if hours_elapsed > 0 else 0
            ),
        }
...
def can_execute_service_operation(
    service: str, estimated_cost: float
) -> tuple[bool, str]:
    """
    ...
    """
    can_execute, reason, _ = per_service_cost_caps.can_execute_operation(
