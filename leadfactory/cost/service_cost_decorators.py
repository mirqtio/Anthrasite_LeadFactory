        ...
        def wrapper(*args, **kwargs):
            ...
            if estimated_cost is not None:
                ...
                try:
                    ...
                    logger.warning(
                        f"Cost calculator failed for {service}.{operation}: {e}"
                    )
                    cost_estimate = 0.0
            else:
                ...
                try:
                    ...
                    estimate_obj = budget_constraints.estimate_operation_cost(
                        service, operation
                    )
                    cost_estimate = estimate_obj.estimated_cost
                except Exception as e:
                    logger.warning(
                        f"Budget constraints estimation failed for {service}.{operation}: {e}"
                    )
                    cost_estimate = 0.0
            ...
            if not can_execute:
                logger.error(
                    f"Service cost cap exceeded for {service}.{operation}: {reason}"
                )
        ...
        def wrapper(*args, **kwargs):
            ...
            if utilization_ratio >= max_utilization:
                ...
                if fallback_function is not None:
                    logger.info("Executing fallback function instead")
...
def gpu_cost_cap(
    operation: str = "processing", cost_calculator: Optional[Callable] = None
):
