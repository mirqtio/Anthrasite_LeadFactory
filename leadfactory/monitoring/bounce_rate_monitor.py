...
class AutomatedBounceMonitor:
    ...
    async def start(self):
        ...
        logger.info(
            f"Started automated bounce rate monitoring (interval: {self.check_interval}s)"
        )
    ...
    async def _check_bounce_rates(self):
        ...
        for ip_subuser_stats in stats:
            ...
            if (
                ip_subuser_stats.total_sent
                < self.bounce_monitor.config.minimum_sample_size
            ):
                continue

            # Check thresholds
            if bounce_rate >= self.bounce_monitor.config.block_threshold:
                await self._handle_block_threshold(
                    ip, subuser, bounce_rate, ip_subuser_stats
                )
            elif bounce_rate >= self.bounce_monitor.config.critical_threshold:
                await self._handle_critical_threshold(
                    ip, subuser, bounce_rate, ip_subuser_stats
                )
            elif bounce_rate >= self.bounce_monitor.config.warning_threshold:
                await self._handle_warning_threshold(
                    ip, subuser, bounce_rate, ip_subuser_stats
                )
            else:
    ...
    async def _get_all_stats(self) -> list[IPSubuserStats]:
        ...
        try:
            ...
            for pool in active_pools:
                for ip_subuser in pool.pool:
                    stats = self.bounce_monitor.get_bounce_rate(
                        ip_subuser.ip_address, ip_subuser.subuser
                    )
                    if stats:
            ...
            for ip_info in warmup_ips:
                stats = self.bounce_monitor.get_bounce_rate(
                    ip_info["ip_address"], ip_info["subuser"]
                )
                if stats:
                    all_stats.append(stats)
    ...
    async def _handle_block_threshold(
        self, ip: str, subuser: str, bounce_rate: float, stats: IPSubuserStats
    ):
        ...
        if warmup_status and warmup_status.status != WarmupStatus.COMPLETED:
            ...
            self.warmup_scheduler.pause_warmup(
                ip, f"Bounce rate exceeded block threshold: {bounce_rate:.2%}"
            )
            await self._send_alert(
                "critical",
                "IP Warmup Paused - Block Threshold",
            ...
            for pool in pools:
                for ip_subuser in pool.pool:
                    if ip_subuser.ip_address == ip and ip_subuser.subuser == subuser:
                        ...
                        ip_subuser.metadata["disabled_reason"] = (
                            f"High bounce rate: {bounce_rate:.2%}"
                        )
                        ...
                        if (
                            hasattr(self.rotation_service, "alerting_service")
                            and self.rotation_service.alerting_service
                        ):
    ...
    async def _handle_critical_threshold(
        ...
        if warmup_status and warmup_status.status == WarmupStatus.IN_PROGRESS:
            ...
            logger.info(
                f"Slowing warmup progression for {ip} due to critical bounce rate"
            )
            ...
            for pool in pools:
                for ip_subuser in pool.pool:
                    if ip_subuser.ip_address == ip and ip_subuser.subuser == subuser:
                        ...
                        ip_subuser.metadata["deprioritized_reason"] = (
                            f"Critical bounce rate: {bounce_rate:.2%}"
                        )
                        ip_subuser.metadata["deprioritized_at"] = (
                            datetime.now().isoformat()
                        )
                        logger.info(
                            f"Deprioritized IP {ip} to priority {ip_subuser.priority}"
                        )
                        break
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response.status != 200:
                            logger.error(
                                f"Failed to send alert webhook: {response.status}"
                            )
            except Exception as e:
...
async def start_automated_monitoring(
    check_interval_seconds: int = 300, alert_webhook_url: Optional[str] = None
