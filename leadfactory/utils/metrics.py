...
if METRICS_AVAILABLE:
    ...
    GPU_PROVISIONING_TIME = Histogram(
        "gpu_provisioning_time_seconds",
        "Time to provision GPU instances",
        ["provider", "instance_type"],
    )
    GPU_COST_HOURLY = Gauge(
        "gpu_cost_hourly_dollars",
        "Hourly cost of GPU instances",
        ["provider", "instance_type"],
    )
    ...
    GPU_SCALING_EVENTS = Counter(
        "gpu_scaling_events_total",
        "Total GPU scaling events",
        ["action", "provider", "instance_type"],
    )
else:
...
def get_queue_metrics(queue_name: str = "personalization") -> Optional[dict]:
    ...
    if queue_name == "personalization":
        try:
            ...
            pending_query = (
                "SELECT COUNT(*) FROM personalization_queue WHERE status = 'pending'"
            )
            processing_query = (
                "SELECT COUNT(*) FROM personalization_queue WHERE status = 'processing'"
            )
            ...
            total_tasks = (
                storage.execute_query(total_query)[0][0]
                if storage.execute_query(total_query)
                else 0
            )
            pending_tasks = (
                storage.execute_query(pending_query)[0][0]
                if storage.execute_query(pending_query)
                else 0
            )
            processing_tasks = (
                storage.execute_query(processing_query)[0][0]
                if storage.execute_query(processing_query)
                else 0
            )

            avg_time_result = storage.execute_query(avg_time_query)
            avg_processing_time = (
                float(avg_time_result[0][0])
                if avg_time_result and avg_time_result[0][0]
                else 300.0
            )
