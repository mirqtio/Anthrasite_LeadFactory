...
@monitoring.command()
@click.option(
    "--interval", default=300, help="Check interval in seconds (default: 300)", type=int
)
@click.option("--webhook-url", help="Webhook URL for alerts", type=str)
    ...
    async def run():
        try:
            monitor = await start_automated_monitoring(
                check_interval_seconds=interval, alert_webhook_url=webhook_url
            )
            ...
            if daemon:
                ...
                try:
                    while True:
                        ...
                        logger.info(
                            f"Monitor status: {status['active_alerts']} active alerts"
                        )
...
@monitoring.command()
@click.option("--ip", help="IP address to check", type=str)
@click.option("--subuser", help="Subuser to check", type=str)
...
@monitoring.command()
@click.option("--days", default=7, help="Number of days to look back", type=int)
