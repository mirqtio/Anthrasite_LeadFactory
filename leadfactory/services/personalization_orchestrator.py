...
from .gpu_manager import GPUInstanceType, gpu_manager
...
class PersonalizationOrchestrator:
    ...
    async def _collect_metrics(self):
        ...
        self.workload_history.append(
            {
                ...
                "gpu_utilization": service_stats["engine_stats"].get(
                    "gpu_utilization", 0.0
                ),
            }
        )
        ...
        self.workload_history = [
            h for h in self.workload_history if h["timestamp"] > cutoff_time
        ]
    ...
    async def _scale_down_recommendation(self):
        ...
        for instance_id, instance_info in gpu_status["instances"].items():
            if (
                instance_info["current_tasks"] == 0
                and instance_info["status"] == "running"
            ):
    ...
    async def _should_start_instance(
        self, instance_type: GPUInstanceType, prediction: WorkloadPrediction
    ) -> bool:
    ...
    async def _route_personalization_tasks(self):
        ...
        for execution_id in active_executions:
            execution_status = self.pipeline_orchestrator.get_execution_status(
                execution_id
            )
            if execution_status and execution_status["status"] == "running":
                ...
                if (
                    "mockup" in stages_completed
                    and "personalization" not in stages_completed
                ):
                    await self._trigger_personalization_stage(
    ...
    async def _trigger_personalization_stage(
        ...
        try:
            ...
            for task_type in task_types:
                ...
                logger.info(
                    f"Submitted {task_type} task {task_id} for execution {execution_id}"
                )
    ...
    async def _update_performance_tracking(self):
        ...
        # Update metrics
        self.metrics["total_tasks_processed"] = service_stats["engine_stats"][
            "tasks_processed"
        ]
        ...
        if len(self.workload_history) >= 2:
            ...
            if len(recent_history) >= 2:
                time_diff = (
                    recent_history[-1]["timestamp"] - recent_history[0]["timestamp"]
                ).total_seconds() / 3600
                task_diff = (
                    recent_history[-1]["processing_tasks"]
                    - recent_history[0]["processing_tasks"]
                )
                if time_diff > 0:
...
class WorkloadPredictor:
    ...
    async def predict_workload(
        ...
        predicted_queue = max(
            0, current_queue + queue_trend * 12
