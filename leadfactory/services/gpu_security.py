...
class CredentialManager:
    ...
    def _get_or_create_encryption_key(self) -> str:
        ...
        try:
            ...
            return base64.urlsafe_b64encode(
                hashlib.sha256(os.environ.get("GPU_ENCRYPTION_SEED", "default").encode()).digest()
            ).decode()
    ...
    def store_credential(self, provider: str, credential_type: str, 
                        credential_value: str, encrypted: bool = True) -> bool:
        """
    ...
    def _update_last_used(self, provider: str, credential_type: str):
        ...
        try:
            if provider in self._credentials_cache:
                if credential_type in self._credentials_cache[provider]:
                    self._credentials_cache[provider][credential_type]["last_used"] = datetime.utcnow().isoformat()
        except Exception as e:
...
class NetworkSecurityManager:
            ...
            return {
                ...
                "Rules": [
                    ...
                    },
                    {
                        ...
                        "FromPort": 8080,
                        "ToPort": 8080,
                        "IpRanges": [{"CidrIp": "10.0.0.0/8", "Description": "GPU worker API"}]
                    }
                ]
            }
    ...
    def validate_instance_security(self, instance_id: str, provider: str) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            validation_results["firewall_configured"] = True  # Assume firewall rules applied
...
class AuditLogger:
        ...
        try:
            ...
            if not os.path.exists(self.log_file):
                with open(self.log_file, 'w') as f:
                    f.write(f"# LeadFactory GPU Audit Log - Created {datetime.utcnow().isoformat()}\n")
    ...
    def log_event(self, event_type: str, details: Dict[str, Any], 
                  user: str = "system", severity: str = "info"):
        """
        ...
        try:
            ...
            if operation == "instance_provision":
                if recent_count >= self.config.max_instances_per_hour:
                    logger.warning(f"Rate limit exceeded for {operation} by {user}: {recent_count} in last hour")
                    return False
...
def validate_gpu_security() -> Dict[str, Any]:
    ...
    try:
        ...
        validation_summary["credentials"]["hetzner"] = credential_manager.validate_credentials("hetzner")
        validation_summary["credentials"]["aws"] = credential_manager.validate_credentials("aws")
        ...
        cred_issues = sum(
            1 for provider_creds in validation_summary["credentials"].values()
        ...
        validation_summary["overall_secure"] = (
            cred_issues == 0 and
            validation_summary["network_security"]["ssh_keys_loaded"] and
            validation_summary["audit_logging"] and
            validation_summary["rate_limiting"]
        )
        
        logger.info(f"GPU security validation: {'PASS' if validation_summary['overall_secure'] else 'FAIL'}")
