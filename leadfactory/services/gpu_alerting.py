...
class GPUAlertManager:
    ...
    def check_budget_alerts(self, current_spend: float, daily_budget: float) -> List[Dict[str, Any]]:
        """
    ...
    def check_instance_alerts(self, gpu_manager_status: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        failed_instances = [
            instance_id for instance_id, instance_data in instances.items()
            ...
            if self._should_send_alert("instance_failure"):
                alerts.append({
                    ...
                    "details": {
                        ...
                        "active_instances": gpu_manager_status.get("active_instances", 0)
                    },
                    "timestamp": datetime.utcnow().isoformat()
                })
    ...
    def check_cost_alerts(self, current_hourly_cost: float, 
                         average_hourly_cost: float) -> List[Dict[str, Any]]:
        """
    ...
    async def send_alert(self, alert: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Log alert
            logger.warning(f"GPU Alert [{alert['severity'].upper()}]: {alert['message']}")
...
async def check_all_gpu_alerts(gpu_manager) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        budget_alerts = gpu_alert_manager.check_budget_alerts(
            cost_tracking["current_spend"],
            cost_tracking["daily_budget"]
        )
