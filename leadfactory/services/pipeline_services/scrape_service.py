...
class ScrapeService(BasePipelineService):
    ...
    async def _process_task(self, request: TaskRequest) -> Dict[str, Any]:
        ...
        logger.info(f"Starting scrape task {request.task_id} for {len(zip_codes)} zip codes")
            ...
            for vertical in verticals:
                try:
                    ...
                    if hasattr(self, '_scrape_yelp'):
                        ...
                        business_ids.extend([b.get("id") for b in yelp_results if b.get("id")])
                    ...
                    if hasattr(self, '_scrape_google'):
                        ...
                        business_ids.extend([b.get("id") for b in google_results if b.get("id")])
                    
                    businesses_scraped += processing_stats["yelp_results"] + processing_stats["google_results"]
        ...
        logger.info(f"Completed scrape task {request.task_id}: {businesses_scraped} businesses")
        return result
