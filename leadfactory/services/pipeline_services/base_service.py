...
class ServiceConfig:
    ...
    postgres_url: str = "postgresql://postgres:postgres@localhost:5432/leadfactory"
...
class BasePipelineService(ABC):
    ...
    def __init__(self, config: ServiceConfig):
        ...
        logger.info(f"Initialized {config.service_name} service v{config.service_version}")
    ...
    def _setup_routes(self):
        ...
        async def health_check():
            ...
            return HealthResponse(
                status="healthy" if all(status == "ok" for status in dependencies.values()) else "degraded",
    ...
    def run(self):
        """Run the service."""
        logger.info(f"Starting {self.config.service_name} service on {self.config.host}:{self.config.port}")
