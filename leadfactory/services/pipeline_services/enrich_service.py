...
class EnrichService(BasePipelineService):
    ...
    def __init__(self, config: Optional[ServiceConfig] = None):
        ...
        if config is None:
            config = ServiceConfig(service_name="enrich", port=8002)
    ...
    async def _check_dependencies(self) -> Dict[str, str]:
        ...
        dependencies["pagespeed_api"] = (
            "ok" if os.getenv("PAGESPEED_API_KEY") else "missing"
        )
        dependencies["screenshot_api"] = (
            "ok" if os.getenv("SCREENSHOT_ONE_API_KEY") else "missing"
        )
    ...
    async def _process_task(self, request: TaskRequest) -> Dict[str, Any]:
        ...
        if not business_ids:
            ...
            if hasattr(self, "_get_businesses"):
                business_ids = await asyncio.get_event_loop().run_in_executor(
                    ...
                    50,  # Limit for service processing
                )
            else:
                raise ValueError("No business IDs provided and enricher not available")

        logger.info(
            f"Starting enrichment task {request.task_id} for {len(business_ids)} businesses"
        )
        ...
        avg_time_per_business = (
            processing_time / len(business_ids) if business_ids else 0
        )
        ...
        logger.info(
            f"Completed enrichment task {request.task_id}: {enriched_count}/{len(business_ids)} successful"
        )
        return result
...
def create_enrich_service() -> EnrichService:
    ...
    config = ServiceConfig(service_name="enrich", port=8002, debug=True)
