...
class PipelineOrchestrator:
    ...
    async def execute_workflow(
        ...
        execution_id = execution_id or f"{workflow_name}_{datetime.utcnow().isoformat()}"
    ...
    async def _execute_stages_sequential(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stage in workflow.stages:
            ...
            for dep in stage.dependencies:
                if dep not in context["stages_completed"]:
                    if stage.required:
                        raise RuntimeError(f"Stage {stage.name} dependency {dep} not satisfied")
                    else:
                        logger.warning(f"Skipping optional stage {stage.name} due to missing dependency {dep}")
                        continue
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if result["status"] == "success":
                            ...
                            raise RuntimeError(f"Stage failed: {result.get('error', 'Unknown error')}")
            
            except Exception as e:
                if attempt < stage.retry_attempts:
                    wait_time = 2 ** attempt  # Exponential backoff
                    logger.warning(f"Stage {stage.name} attempt {attempt + 1} failed, retrying in {wait_time}s: {e}")
                    await asyncio.sleep(wait_time)
                else:
                    raise RuntimeError(f"Stage {stage.name} failed after {stage.retry_attempts} retries: {e}")
    ...
    async def health_check_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        for service_name, service_info in self.registry.list_services().items():
            try:
                health_url = f"http://{service_info['host']}:{service_info['port']}/health"
