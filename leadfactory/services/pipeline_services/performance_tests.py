...
class PipelineLoadTester:
    ...
    async def test_service_load(
        ...
        logger.info(
            f"Target: {total_requests} requests with {concurrent_requests} concurrent"
        )
    ...
    async def test_workflow_performance(
        self, concurrent_workflows: int = 50, total_workflows: int = 200
    ) -> PerformanceMetrics:
        """
        ...
        Returns:
            Performance metrics
        """
        logger.info("Starting workflow performance test")
        logger.info(
            f"Target: {total_workflows} workflows with {concurrent_workflows} concurrent"
        )
        ...
        workflows_per_second = (
            total_workflows / total_duration if total_duration > 0 else 0
        )
        ...
        logger.info("Workflow performance test completed")
        logger.info(
            f"Workflows/sec: {workflows_per_second:.2f}, Error Rate: {error_rate:.2%}"
        )
...
class CapacityValidator:
    ...
    def validate_capacity_improvement(
        ...
        rps_improvement = (
            ...
            if baseline_metrics.requests_per_second > 0
            else float("inf")
        )
        ...
        overall_success = (
            rps_target_met and response_time_acceptable and error_rate_acceptable
        )
    ...
    def _generate_recommendations(
        ...
        if not recommendations:
            recommendations.append(
                "Performance targets met! System is ready for production."
            )
...
async def run_comprehensive_performance_tests():
    ...
    try:
        ...
        workflow_metrics = await load_tester.test_workflow_performance(
            concurrent_workflows=20, total_workflows=100
        )
        ...
        report = {
            "timestamp": time.time(),
            "test_summary": {"total_tests": len(results), "test_results": {}},
            "detailed_results": {},
        }
        ...
        logger.info(
            "Performance testing completed. Report saved to performance_test_report.json"
        )
