...
try:
    from leadfactory.utils.metrics import (
        GPU_INSTANCES_ACTIVE, GPU_QUEUE_SIZE, GPU_PROVISIONING_TIME,
        GPU_COST_HOURLY, GPU_UTILIZATION, GPU_SCALING_EVENTS, record_metric
    )
...
try:
    from leadfactory.services.gpu_security import (
        credential_manager, network_security, audit_logger, rate_limiter
    )
...
class HetznerAPIClient:
    ...
    def create_server(self, name: str, server_type: str, image: str, 
                     location: str = "nbg1", ssh_keys: List[str] = None, 
...
class GPUAutoSpinManager:
        ...
        if HETZNER_AVAILABLE:
            try:
                ...
                if SECURITY_AVAILABLE:
                    hetzner_token = credential_manager.get_credential("hetzner", "api_token")
                else:
    ...
    async def _evaluate_scaling_needs(self):
        ...
        current_utilization = self.queue_metrics.processing_tasks / max(total_capacity, 1)
        
        # Decision logic for scaling - Updated for Task 22 requirements
        should_scale_up = (
            self.queue_metrics.pending_tasks > 2000 and  # Large queue (2000 threshold per requirements)
            current_utilization > 0.8 and  # High utilization
            self.queue_metrics.estimated_completion_time > 1800 and  # >30min ETA
            self._within_budget_to_scale() and
            self._can_scale_up()  # Check cooldown period
        )
        
        should_scale_down = (
            self.queue_metrics.pending_tasks < 100 and  # Small queue (updated threshold)
            current_utilization < 0.2 and  # Low utilization
            len(self.active_instances) > 1 and  # Keep at least one instance
            self._can_scale_down()  # Check cooldown period
        )
    ...
    def _within_budget_to_scale(self) -> bool:
        ...
        daily_remaining = self.cost_tracking["daily_budget"] - self.cost_tracking["current_spend"]
        
        # Estimate cost for next instance (assume 4 hours)
        cheapest_instance = min(
            self.resource_configs.values(),
    ...
    async def _scale_up(self):
        ...
        running_types = [
            inst.instance_type for inst in self.active_instances.values()
            if inst.status == "running"
        ]
        ...
        if instance_type not in running_types:
            ...
            if instance_id:
                ...
                logger.info(f"Scaled up: Started {instance_type.value} instance {instance_id}")
                
                # Record scaling event
                if METRICS_AVAILABLE:
                    provider = "hetzner" if instance_type.value.startswith("hetzner") else "aws"
                    record_metric(GPU_SCALING_EVENTS, 1, action="scale_up", provider=provider, instance_type=instance_type.value)
    ...
    async def _scale_down(self):
        ...
        if running_instances:
            ...
            # Record scaling event
            if METRICS_AVAILABLE:
                provider = "hetzner" if instance_to_stop.instance_type.value.startswith("hetzner") else "aws"
                record_metric(GPU_SCALING_EVENTS, 1, action="scale_down", provider=provider, instance_type=instance_to_stop.instance_type.value)
    ...
    async def start_gpu_instance(self, instance_type: GPUInstanceType) -> Optional[str]:
        ...
        if SECURITY_AVAILABLE:
            ...
            if not rate_limiter.check_rate_limit("instance_provision"):
                logger.warning(f"Rate limit exceeded for instance provisioning")
                audit_logger.log_event(
                    "rate_limit_exceeded",
                    {"operation": "instance_provision", "instance_type": instance_type.value},
                    severity="warning"
                )
                return None
            ...
            if instance_type == GPUInstanceType.LOCAL_GPU:
                ...
                if SECURITY_AVAILABLE:
                    audit_logger.log_event(
                        ...
                        {"reason": "unsupported_instance_type", "instance_type": instance_type.value},
                        severity="warning"
                    )
                return None
    ...
    async def _start_aws_instance(self, instance_type: GPUInstanceType) -> Optional[str]:
                    ...
                    "TagSpecifications": [
                        {
                            ...
                            "Tags": [
                                {"Key": "Name", "Value": f"leadfactory-gpu-{instance_type.value}"},
    ...
    async def _start_hetzner_instance(self, instance_type: GPUInstanceType) -> Optional[str]:
...
"""
    ...
    async def stop_gpu_instance(self, instance_id: str):
        ...
        try:
            ...
            if instance.start_time:
                runtime_hours = (datetime.utcnow() - instance.start_time).total_seconds() / 3600
    ...
    async def _stop_hetzner_instance(self, instance_id: str):
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(
                None,
                self.hetzner_client.delete_server,
                instance_id
            )
    ...
    async def _update_aws_instance_status(self, instance: GPUInstance):
        ...
        try:
            ...
            logger.error(f"Failed to update instance {instance.instance_id} status: {e}")
    ...
    async def _update_hetzner_instance_status(self, instance: GPUInstance):
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                self.hetzner_client.get_server,
            ...
            instance.status = status_mapping.get(hetzner_server["status"], hetzner_server["status"])
            
            if instance.status == "running" and not instance.ip_address:
                instance.ip_address = hetzner_server["public_net"]["ipv4"]["ip"]
                
        except Exception as e:
            logger.error(f"Failed to update Hetzner instance {instance.instance_id} status: {e}")
    ...
    async def _log_status(self):
        ...
        if METRICS_AVAILABLE:
            # Update queue metrics
            record_metric(GPU_QUEUE_SIZE, self.queue_metrics.pending_tasks, status="pending")
            record_metric(GPU_QUEUE_SIZE, self.queue_metrics.processing_tasks, status="processing")
            ...
            for instance in self.active_instances.values():
                if instance.status == "running":
                    provider = "hetzner" if instance.instance_type.value.startswith("hetzner") else "aws"
            ...
            for (provider, instance_type), count in instance_counts.items():
                record_metric(GPU_INSTANCES_ACTIVE, count, provider=provider, instance_type=instance_type)
                record_metric(GPU_COST_HOURLY, hourly_costs[(provider, instance_type)], provider=provider, instance_type=instance_type)
