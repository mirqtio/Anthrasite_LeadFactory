...
class IPPoolManager:
    ...
    async def _check_pool_health(self):
        ...
        for pool_id, pool in self.ip_pools.items():
            if pool.status in [IPPoolStatus.QUARANTINED, IPPoolStatus.DISABLED]:
                ...
                if (pool.quarantine_until and 
                    datetime.utcnow() > pool.quarantine_until):
            ...
            if pool.bounce_rate > self.thresholds["bounce_rate_critical"]:
                ...
                logger.warning(f"Pool {pool_id} bounce rate warning: {pool.bounce_rate:.3f}")
            ...
            if pool.spam_rate > self.thresholds["spam_rate_critical"]:
                ...
                pool.status = IPPoolStatus.WARNING
                logger.warning(f"Pool {pool_id} spam rate warning: {pool.spam_rate:.3f}")
            ...
            # Check daily volume
            if pool.daily_volume > pool.max_daily_volume * self.thresholds["volume_threshold"]:
                pool.status = IPPoolStatus.WARNING
                logger.warning(f"Pool {pool_id} approaching volume limit: {pool.daily_volume}/{pool.max_daily_volume}")
    ...
    async def _process_bounce_events(self):
        ...
        recent_bounces = [
            event for event in self.bounce_events
    ...
    async def _evaluate_pool_switching(self):
        ...
        should_switch = (
            current_pool.status == IPPoolStatus.QUARANTINED or
            current_pool.status == IPPoolStatus.DISABLED or
            current_pool.bounce_rate > self.thresholds["bounce_rate_critical"] or
            current_pool.spam_rate > self.thresholds["spam_rate_critical"] or
            current_pool.daily_volume >= current_pool.max_daily_volume
        )
    ...
    async def _select_best_pool(self) -> Optional[str]:
        ...
        available_pools = [
            (pool_id, pool) for pool_id, pool in self.ip_pools.items()
            if pool.status == IPPoolStatus.ACTIVE and 
            pool.daily_volume < pool.max_daily_volume
        ]
        ...
        if not available_pools:
            ...
            available_pools = [
                (pool_id, pool) for pool_id, pool in self.ip_pools.items()
                if pool.status == IPPoolStatus.WARNING and 
                pool.daily_volume < pool.max_daily_volume
            ]
    ...
    async def _update_pool_metrics(self):
        ...
        try:
            ...
            for pool_id, pool in self.ip_pools.items():
                ...
                pool.reputation_score = max(0.0, min(1.0, 
                    1.0 - (pool.bounce_rate * 5) - (pool.spam_rate * 10)
                ))
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_pools": len([p for p in self.ip_pools.values() if p.status == IPPoolStatus.ACTIVE]),
            "quarantined_pools": len([p for p in self.ip_pools.values() if p.status == IPPoolStatus.QUARANTINED]),
            "recent_bounces": len([e for e in self.bounce_events if e.timestamp > datetime.utcnow() - timedelta(hours=1)]),
    ...
    def get_pool_details(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            pool_id: {
                ...
                "quarantine_until": pool.quarantine_until.isoformat() if pool.quarantine_until else None
            }
            for pool_id, pool in self.ip_pools.items()
        }
