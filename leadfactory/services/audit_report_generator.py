...
class AuditReportGenerator:
    ...
    async def generate_audit_report(
        ...
        try:
            logger.info(
                f"Generating audit report for {business_name} (ID: {report_id})"
            )
            ...
            if not business_data:
                logger.warning(
                    f"No business data found for {business_name}, generating minimal report"
                )
            ...
            if output_path or return_bytes:
                ...
                result = self.pdf_generator.generate_document(
                    content=report_content, output_path=temp_path, return_bytes=False
                )
    ...
    async def _fetch_business_data(
        ...
        try:
            with self.storage.cursor() as cursor:
                ...
                if business_data.get("tech_stack"):
                    try:
                        business_data["tech_stack"] = json.loads(
                ...
                if business_data.get("semrush_json"):
                    try:
                        business_data["semrush_data"] = json.loads(
                            business_data["semrush_json"]
                        )
                    except json.JSONDecodeError:
    ...
    async def _generate_audit_analysis(
        ...
        if business_data.get("semrush_data"):
            ...
            if isinstance(semrush, dict):
                if semrush.get("errors"):
                    prompt_parts.append(
                        f"SEO Issues: {len(semrush['errors'])} issues found"
                    )
                if semrush.get("warnings"):
                    prompt_parts.append(
                        f"SEO Warnings: {len(semrush['warnings'])} warnings"
                    )
    ...
    def _parse_analysis_response(
        self, analysis_text: str, business_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _generate_fallback_analysis(
        ...
        if page_speed > 0:
            if page_speed >= 90:
                findings.append(
                    f"Excellent website performance with PageSpeed score of {page_speed}/100"
                )
            elif page_speed >= 70:
                findings.append(
                    f"Good website performance with PageSpeed score of {page_speed}/100"
                )
                recommendations.append(
                    "Consider optimizing images and reducing JavaScript to improve load times"
                )
            else:
                findings.append(
                    f"Website performance needs improvement with PageSpeed score of {page_speed}/100"
                )
                recommendations.append(
                    "Implement performance optimizations: image compression, code minification, and caching"
                )
        ...
        if tech_stack:
            ...
            if any(tech in str(tech_stack) for tech in modern_techs):
                ...
                recommendations.append(
                    "Consider upgrading to modern web technologies for better performance and SEO"
                )
        ...
        if isinstance(semrush_data, dict):
            ...
            if errors:
                findings.append(
                    f"SEO audit identified {len(errors)} critical issues requiring attention"
                )
                recommendations.append(
                    "Address critical SEO issues to improve search engine visibility"
                )

            if warnings:
                findings.append(
                    f"SEO audit found {len(warnings)} optimization opportunities"
                )
                recommendations.append(
                    "Implement SEO best practices to enhance search rankings"
                )
    ...
    def _build_report_content(
        ...
        content.extend(
            [
                ...
                {
                    "type": "paragraph",
                    "text": audit_analysis.get(
                        "executive_summary", "Audit completed successfully."
                    ),
                    "style": "CustomBodyText",
                },
                {"type": "spacer", "height": 20},
        ...
        for source in data_sources:
            content.append(
                {"type": "paragraph", "text": f"• {source}", "style": "CustomBodyText"}
            )
    ...
    def _build_technical_analysis_table(
        self, business_data: dict[str, Any]
    ) -> list[list[str]]:
    ...
    async def _generate_minimal_report(
        ...
        if output_path or return_bytes:
            return self.pdf_generator.generate_document(
                content=content, output_path=output_path, return_bytes=return_bytes
            )
        else:
            # Generate to temporary file
            temp_dir = tempfile.mkdtemp()
            temp_path = os.path.join(temp_dir, f"{report_id}.pdf")
            return self.pdf_generator.generate_document(
                content=content, output_path=temp_path, return_bytes=False
            )
