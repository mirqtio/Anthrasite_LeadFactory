...
"""
    ...
    def _parse_response(self, response_text: str) -> tuple[bool, float, str]:
        ...
        duplicate_match = re.search(
            r"DUPLICATE:\s*(Union[YES, NO])", response_text, re.IGNORECASE
        )
...
def get_potential_duplicates(limit: Optional[int] = None) -> list[Dict]:
    ...
    try:
        with get_database_connection() as cursor:
            ...
            """
            if limit:
                # Use parameterized query for the limit to prevent SQL injection
                query += " LIMIT ?"
...
def flag_for_review(
    ...
    try:
        with get_database_connection() as cursor:
            ...
            logger.info(
                f"Flagged businesses {business1_id} and {business2_id} "
                f"for manual review: {reason}"
            )
    except Exception as e:
        logger.error(
            f"Error flagging businesses {business1_id} and {business2_id} "
            f"for review: {e}"
        )
...
def process_duplicate_pair(
    ...
    if is_duplicate and confidence >= 0.7:  # Require high confidence for merging
        ...
        if (
            business1["name"] == business2["name"]
            and business1["address"] != business2["address"]
        ):
            if not is_dry_run:
                flag_for_review(
                    business1_id,
...
def main():
    ...
    for duplicate_pair in duplicate_pairs:
        ...
        try:
            ...
            error_count += 1
    logger.info(
        f"Deduplication completed. Success: {success_count}, Errors: {error_count}, Skipped: {skipped_count}"
    )
    return 0
